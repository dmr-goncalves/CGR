...
public class SwitchCommands 
{
    ...
    public static boolean installRule(IOFSwitch sw, TableId table, short priority,
            ...
            short hardTimeout, short idleTimeout, OFBufferId bufferId)
    {
			...
			OFPacketIn pi) 
    {
        ...
        {
            outSw.write(pktOut.build());
            log.info("Forwarding packet out: "+pi.toString() + "to Switch: " + outSw );
        }
        catch (Exception e) 
        {
        	log.error("Failed to forward packet: "+pi.toString() + "to Switch: " + outSw);
			return false;
        }
